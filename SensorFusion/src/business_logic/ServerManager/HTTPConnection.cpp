...
HTTPConnection::HTTPConnection(const std::string& serverAddr, uint16_t serverPort, CryptoMng* cryptoMng, bool enableSSL) : serverAddr(serverAddr), serverPort(serverPort), cryptoMng(cryptoMng), enableSSL(enableSSL)
{
...
SocketHandler* HTTPConnection::getSocket()
{
...
{
	socket = getSocket();
	socket->connectSocket(serverAddr.c_str(),serverPort);
...
std::string HTTPConnection::request(const std::string& getReq)
{
	...
	{
		LOG_WARNING("Functional error, retry server request: ", e.what());
	}
	return response;
}
