...
size_t readDataCallback(void *ctx, unsigned char *buf, size_t len)
{
	...
	SocketHandler* socket = cryptoMng->getSocket();
...
void CryptoMng::handshakeSSL()
{
	...
	LOG_INFO("Getting certificate from server");
...
void CryptoMng::certificateVerification()
{
	...
	if (flags != ERR_OK)
	{
		...
		LOG_DEBUG("Certificate verification failed: ", serverCert);
...
void CryptoMng::sendData(const std::string& getReq)
{
	...
	if ((ret > 0) && (static_cast<size_t>(ret) != dataLenght))
	{
		...
		LOG_WARNING("The full request was not sent! Pending byte: ", (dataLenght - pendingData));
...
void CryptoMng::configureSSL()
{
	...
	if (ret != ERR_OK)
	{
		LOG_ERROR("Error configuring default SSL configuration: ",-ret);
		//THROW exception???
	}
	...
	{
		LOG_ERROR("Error during Secure socket layer ", -ret);
		//THROW exception
	}
	...
	if(cryptoCfg.checkHostName)
	{
		...
		{
			LOG_DEBUG("Error assigning the server host name to the crypto layer ");
			//THROW exception
		}
	}
...
void CryptoMng::configureEntropy()
{
	...
	if (ret != ERR_OK)
	{
		...
		LOG_ERROR("Error during entropy configuration ", -ret);
	}
#endif
}
...
int CryptoMng::crtfVerify(void *ctx, mbedtls_x509_crt *crt, int depth,uint32_t *flags)
{
	...
	if (ret < 0)
	{
		LOG_DEBUG("Error getting server certificate information");
	}
	...
	LOG_DEBUG("Checking the parsed certificate flags: ", *flags, "  against the previous one:  ", doubleCheck);
	...
	if (doubleCheck != ERR_OK)
	{
		...
		if (ret < 0)
		{
			LOG_DEBUG("mbedtls_x509_crt_verify_info() returned ", -ret);
		}
		else
		{
			LOG_DEBUG("Certificate verification failed -> flags: ", flags);
		}
	}
...
SocketHandler* CryptoMng::getSocket()
{
#ifndef TEST_BUILD
